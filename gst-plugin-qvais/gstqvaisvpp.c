...
void *
qvaisvpp_init (uint32_t flags, struct vpp_callbacks cb)
{
...
gboolean
qvaisvpp_set_ctrl (void *ctx, struct hqv_control ctrl)
{
  gboolean ret = FALSE;
  enum vpp_error vpp_ret = VPP_OK;
...
gboolean
qvaisvpp_set_parameter (void *ctx, enum vpp_port port,
    struct vpp_port_param param)
{
  ...
  GST_DEBUG
      ("VPP set param ctx:%p, port %d, fmt %d, width %d, height %d, stride %d, scanlines %d",
      ctx, port, param.fmt, param.width, param.height, param.stride,
      param.scanlines);
...
gboolean
qvaisvpp_get_buf_requirements (void *ctx, struct vpp_requirements *req)
{
  gboolean ret = FALSE;
  enum vpp_error vpp_ret = VPP_OK;
...
gboolean
qvaisvpp_queue_buf (void *ctx, enum vpp_port port, struct vpp_buffer *buf)
{
  gboolean ret = FALSE;
  enum vpp_error vpp_ret = VPP_OK;
  ...
  GST_DEBUG ("VPP queue buf, port=%u, vpp buf=%p, gst buf=%p", port, buf,
      gst_buf);

  vpp_ret = vpp_queue_buf (ctx, port, buf);
  if (vpp_ret == VPP_OK) {
    GST_DEBUG ("qvaisvpp_queue_buf queue buffer success");
    ret = TRUE;
  } else
...
gboolean
qvaisvpp_flush (void *ctx, enum vpp_port port)
{
  gboolean ret = FALSE;
  enum vpp_error vpp_ret = VPP_OK;
