...
gboolean
qvais_dmabuf_alloc (DmaBufDesc ** desc,
    const GstVideoInfo * info, gboolean ubwc)
{
  GST_INFO ("ubwc %u, size %" G_GSIZE_FORMAT, ubwc, GST_VIDEO_INFO_SIZE (info));
  ...
  *desc = g_new0 (DmaBufDesc, 1);
  ...
  GST_DEBUG ("desc %p, size %" G_GSIZE_FORMAT,
      *desc, qvais_dmabuf_get_size (*desc));
...
gint
qvais_dmabuf_get_fd (const DmaBufDesc * desc)
{
  gint fd = -1;
...
gsize
qvais_dmabuf_get_size (const DmaBufDesc * desc)
{
  gsize size = 0;
...
guint64
qvais_dmabuf_get_modifier (const DmaBufDesc * desc)
{
...
void
qvais_dmabuf_align_info (const DmaBufDesc * desc, GstVideoInfo * info)
{
  ...
  GST_DEBUG ("aligned info stride %u, offset0 %u, offset1 %u, size %lu",
      desc->stride, gbm_bo_get_offset (desc->bo, 0),
      gbm_bo_get_offset (desc->bo, 1), desc->size);
}

void
qvais_dmabuf_free (DmaBufDesc * desc)
{
