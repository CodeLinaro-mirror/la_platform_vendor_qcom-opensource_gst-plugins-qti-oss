...
GST_DEBUG_CATEGORY_EXTERN (gst_qvais_debug);
#define GST_CAT_DEFAULT gst_qvais_debug

/* Below is a simple GstQvaisMeta implementation. */
...
#define GST_QVAIS_META_API_TYPE (gst_qvais_meta_api_get_type())
#define GST_QVAIS_META_INFO     (gst_qvais_meta_get_info())

#define gst_buffer_get_qvais_meta(b) \
    ((GstQvaisMeta *)gst_buffer_get_meta((b),GST_QVAIS_META_API_TYPE))

static GType
gst_qvais_meta_api_get_type (void)
{
  static GType type = 0;

  if (g_once_init_enter (&type)) {
    static const gchar *tags[] = { NULL };
    GType _type = gst_meta_api_type_register ("GstQvaisMetaAPI", tags);
...
static gboolean
gst_qvais_meta_init (GstMeta * meta, gpointer params, GstBuffer * buffer)
{
  GstQvaisMeta *emeta = (GstQvaisMeta *) meta;
...
gst_qvais_meta_get_info (void)
{
  static const GstMetaInfo *meta_info = NULL;

  if (g_once_init_enter (&meta_info)) {
    const GstMetaInfo *mi = gst_meta_register (GST_QVAIS_META_API_TYPE,
        "GstQvaisMeta", sizeof (GstQvaisMeta),
        gst_qvais_meta_init, NULL, NULL);
...
static GstQvaisMeta *
_add_qvais_meta (GstBuffer * buffer, const DmaBufDesc * desc)
{
  GstQvaisMeta *meta;
  ...
  meta = (GstQvaisMeta *) gst_buffer_add_meta (buffer,
      GST_QVAIS_META_INFO, NULL);
...
/* Below is GstQvaisPool implementation. */
static GstBufferPoolClass *q_parent_class = NULL;

G_DEFINE_TYPE (GstQvaisPool, gst_qvais_pool, GST_TYPE_BUFFER_POOL);

static gboolean
gst_qvais_pool_set_config (GstBufferPool * pool, GstStructure * config)
{
  GstQvaisPool *self = GST_QVAIS_POOL (pool);
  GstVideoInfo info;
  GstCaps *caps;
  guint size, min, max;
...
_modifier_attach (GstBuffer * buffer, const DmaBufDesc * desc)
{
  ...
  *modifier = qvais_dmabuf_get_modifier (desc);
    ...
    DmaBufDesc ** desc, const GstVideoInfo * info, gboolean ubwc)
{
  ...
  if (!qvais_dmabuf_alloc (desc, info, ubwc)) {
    GST_ERROR_OBJECT (allocator, "alloc error");
    goto out;
  }

  fd = qvais_dmabuf_get_fd (*desc);
  size = qvais_dmabuf_get_size (*desc);
...
gst_qvais_pool_aligned_info (const GstBufferPool * pool)
{
  GstQvaisPool *self = GST_QVAIS_POOL ((GstBufferPool *) pool);

  return &self->aligned_info;
}

static GstFlowReturn
gst_qvais_pool_alloc (GstBufferPool * pool, GstBuffer ** buffer,
    GstBufferPoolAcquireParams * params)
{
  ...
  *buffer = gst_buffer_new ();
  ...
  if (!self->align_info_done) {
    *ainfo = *info;
    qvais_dmabuf_align_info (desc, ainfo);
    self->align_info_done = TRUE;
  }
...
do_dmabuf_free (GstBuffer * buffer)
{
  ...
  GstQvaisMeta *meta = gst_buffer_get_qvais_meta (buffer);
  DmaBufDesc *desc;
  gint desc_fd;
  ...
  desc_fd = qvais_dmabuf_get_fd (desc);
...
static void
gst_qvais_pool_free (GstBufferPool * pool, GstBuffer * buffer)
{
...
GstBufferPool *
gst_qvais_pool_new (gboolean ubwc)
{
  GstQvaisPool *pool;

  pool = g_object_new (GST_TYPE_QVAIS_POOL, NULL);
  pool->ubwc = ubwc;
...
static void
gst_qvais_pool_finalize (GObject * object)
{
  GstQvaisPool *pool = GST_QVAIS_POOL (object);
...
static void
gst_qvais_pool_class_init (GstQvaisPoolClass * klass)
{
  ...
  gobject_class->finalize = GST_DEBUG_FUNCPTR (gst_qvais_pool_finalize);

  bpool_class->set_config = GST_DEBUG_FUNCPTR (gst_qvais_pool_set_config);
  bpool_class->alloc_buffer = GST_DEBUG_FUNCPTR (gst_qvais_pool_alloc);
  bpool_class->free_buffer = GST_DEBUG_FUNCPTR (gst_qvais_pool_free);
}

static void
gst_qvais_pool_init (GstQvaisPool * pool)
{
  GST_INFO_OBJECT (pool, "pool %p", pool);
