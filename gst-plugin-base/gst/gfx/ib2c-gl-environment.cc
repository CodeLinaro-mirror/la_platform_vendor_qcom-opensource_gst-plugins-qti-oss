...
std::string Environment::NewEnvironment(
      std::shared_ptr<Environment>& environment, bool is_secure) {

  environment = std::make_shared<Environment>();
  return environment->Initialize(is_secure);
}
...
std::string Environment::Initialize(bool is_secure) {
  ...
  if (is_secure && QueryExtension("EGL_EXT_protected_content")) {
