...
static inline gboolean
_ext_dmabuf_fill_desc (DmaBufDesc * desc,
    const GstVideoInfo * info, gboolean ubwc)
{
#ifdef USE_GBM
...
static inline gboolean
_ext_dmabuf_alloc (DmaBufDesc * desc)
{
#ifdef USE_GBM
...
static inline void
_ext_dmabuf_free (DmaBufDesc * desc)
{
#ifdef USE_GBM
...
gboolean
ext_dmabuf_alloc (DmaBufDesc ** desc,
    const GstVideoInfo * info, gboolean ubwc)
{
  ...
  *desc = g_new0 (DmaBufDesc, 1);
  ...
  if (!_ext_dmabuf_fill_desc (*desc, info, ubwc))
    goto desc_free;
...
gint
ext_dmabuf_get_fd (const DmaBufDesc * desc)
{
  gint fd = -1;
...
gsize
ext_dmabuf_get_size (const DmaBufDesc * desc)
{
  gsize size = 0;
...
guint64
ext_dmabuf_get_modifier (const DmaBufDesc * desc)
{
...
void
ext_dmabuf_align_info (const DmaBufDesc * desc, GstVideoInfo * info)
{
...
void
ext_dmabuf_free (DmaBufDesc * desc)
{
  GST_DEBUG ("desc %p", desc);

  _ext_dmabuf_free (desc);
  g_free (desc);
