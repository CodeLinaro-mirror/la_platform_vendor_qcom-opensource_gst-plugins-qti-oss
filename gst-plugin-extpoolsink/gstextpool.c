...
GST_DEBUG_CATEGORY_EXTERN (gst_ext_pool_sink_debug);
#define GST_CAT_DEFAULT gst_ext_pool_sink_debug

/* Below is a simple GstExtPoolMeta implementation. */
...
#define GST_EXTPOOL_META_API_TYPE (gst_ext_meta_api_get_type())
#define GST_EXTPOOL_META_INFO     (gst_ext_meta_get_info())

#define gst_buffer_get_ext_pool_meta(b) \
    ((GstExtPoolMeta *)gst_buffer_get_meta((b),GST_EXTPOOL_META_API_TYPE))

static GType
gst_ext_meta_api_get_type (void)
{
  static GType type = 0;

  if (g_once_init_enter (&type)) {
    static const gchar *tags[] = { NULL };
    GType _type = gst_meta_api_type_register ("GstExtPoolMetaAPI", tags);
...
static gboolean
gst_ext_meta_init (GstMeta * meta, gpointer params, GstBuffer * buffer)
{
  GstExtPoolMeta *emeta = (GstExtPoolMeta *) meta;
...
gst_ext_meta_get_info (void)
{
  static const GstMetaInfo *meta_info = NULL;

  if (g_once_init_enter (&meta_info)) {
    const GstMetaInfo *mi = gst_meta_register (GST_EXTPOOL_META_API_TYPE,
        "GstExtPoolMeta", sizeof (GstExtPoolMeta),
        gst_ext_meta_init, NULL, NULL);
...
static GstExtPoolMeta *
_add_ext_meta (GstBuffer * buffer, const DmaBufDesc * desc)
{
  GstExtPoolMeta *meta;
  ...
  meta = (GstExtPoolMeta *) gst_buffer_add_meta (buffer,
      GST_EXTPOOL_META_INFO, NULL);
...
/* Below is GstExtPool implementation. */

#define gst_ext_pool_parent_class parent_class
G_DEFINE_TYPE (GstExtPool, gst_ext_pool, GST_TYPE_BUFFER_POOL);

#if 0
static const gchar **
gst_ext_pool_get_options (GstBufferPool * pool)
{
...
static gboolean
gst_ext_pool_set_config (GstBufferPool * pool, GstStructure * config)
{
  GstExtPool *self = GST_EXT_POOL (pool);
  GstVideoInfo info;
  GstCaps *caps;
  guint size, min, max;
...
_modifier_attach (GstBuffer * buffer, const DmaBufDesc * desc)
{
  ...
  *modifier = ext_dmabuf_get_modifier (desc);
...
/*
static gboolean
gst_is_ext_pool_buffer(const GstBufferPool * pool, const GstBuffer * buffer)
{
  gboolean ret = FALSE;
  const gchar *str = "no";
    ...
    DmaBufDesc ** desc, const GstVideoInfo * info, gboolean ubwc)
{
  ...
  if (!ext_dmabuf_alloc (desc, info, ubwc)) {
    GST_ERROR_OBJECT (allocator, "alloc error");
    goto out;
  }

  fd = ext_dmabuf_get_fd (*desc);
  size = ext_dmabuf_get_size (*desc);
...
gst_ext_pool_alloc (GstBufferPool * pool, GstBuffer ** buffer,
    GstBufferPoolAcquireParams * params)
{
  ...
  *buffer = gst_buffer_new ();
  ...
  if (!self->done_align_info) {
    *ainfo = *info;
    ext_dmabuf_align_info (desc, ainfo);
...
do_dmabuf_free (GstBuffer * buffer)
{
  ...
  GstExtPoolMeta *meta = gst_buffer_get_ext_pool_meta (buffer);
  DmaBufDesc *desc;
  gint desc_fd;
  ...
  desc_fd = ext_dmabuf_get_fd (desc);
...
static void
gst_ext_pool_free (GstBufferPool * pool, GstBuffer * buffer)
{
...
GstBufferPool *
gst_ext_pool_new (gboolean ubwc)
{
  GstExtPool *pool;

  pool = g_object_new (GST_TYPE_EXT_POOL, NULL);
...
static void
gst_ext_pool_finalize (GObject * object)
{
  GstExtPool *pool = GST_EXT_POOL (object);
...
static void
gst_ext_pool_class_init (GstExtPoolClass * klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstBufferPoolClass *bpool_class = (GstBufferPoolClass *) klass;

  GST_INFO ("klass %p", klass);
  gobject_class->finalize = GST_DEBUG_FUNCPTR (gst_ext_pool_finalize);

  //bpool_class->get_options = gst_ext_pool_get_options;
  bpool_class->set_config = GST_DEBUG_FUNCPTR (gst_ext_pool_set_config);
  bpool_class->alloc_buffer = GST_DEBUG_FUNCPTR (gst_ext_pool_alloc);
  bpool_class->free_buffer = GST_DEBUG_FUNCPTR (gst_ext_pool_free);
}

static void
gst_ext_pool_init (GstExtPool * pool)
{
  GST_INFO_OBJECT (pool, "pool %p", pool);
